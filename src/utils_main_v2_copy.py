...
def load_data(hyperparams, CL = False, sample_rate = 1):
    ...
    if CL:
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=hyperparams['batch_size'], shuffle=True, collate_fn=CL_collate_v2)
    else:
    ...
    if CL:
        test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=hyperparams['batch_size'], shuffle=True, collate_fn=CL_collate_v2)
    else:
